...
- First quantitative exposure of predictor length bias on the iDPPIV-SCM homologous benchmark.
...
> Note: the above numbers were generated by `scripts/mmgbsa_top3.py` into `data/phaseB/mmgbsa_top3.tsv` (pocket-restricted implicit GBn2 single-trajectory MM-GBSA, see §2.6). MM-GBSA serves only as the **lowest-weight, qualitative vote** in the multi-signal consensus (§3.7): it is a pocket-restricted single-structure approximation whose absolute values for 5-residue peptides are noisy and sensitive to pocket radius and single- vs multi-conformation sampling (Wang et al., 2019; Blanes-Mira et al., 2023); it must not, on its own, overturn the Vina ranking. Absolute values are further affected by single-conformation, pocket truncation, and ignored −TΔS — interpretation in §4.3.
...
> **Consolidated-ranking conclusion:** **LPPGP > APPSQ >> APQIP**. This priority is decided by **multi-signal consensus** (composition + docking + contact profile + independent predictor), not by any single layer; among these, MM-GBSA carries the **lowest weight** (§3.5, §4.3). Note: this priority derives from the DPP4 single-target framework; the §3.11 multi-target extension supplies LPPGP broader antidiabetic-spectrum evidence. All metrics are computational products; final priority awaits (any future) wet-lab IC₅₀ validation (see §4.3).
...
**Background and motivation.** On the iDPPIV-SCM homologous public benchmark (train 1,063 / test 266, balanced positive/negative) and the moso candidate short-peptide pool (n=4,950, 2–6 aa), we implemented the §2.8 protocol and provide the first quantitative evidence of length bias on this homologous benchmark.
...
#### 3.9.2 Length-agnostic ranking of the three finalists (robustness check)

To test whether the finalist priority merely rides the cross-length "shorter = higher" effect, we re-ranked the three finalists **only within their own 5-residue length class** of the candidate pool (n = 1,202 pentapeptides), removing the cross-length length signal entirely (script `scripts/phaseA/length_agnostic_ranking.py`, data `data/phaseA/length_agnostic_ranking.tsv`).

| Finalist | Length | iDPPIV score | Rank in 5-mer class | Top % in class | z vs class mean |
|---|---|---|---|---|---|
| LPPGP | 5 | 2.452 | 1196 / 1202 | 99.4% | +2.45 |
| APQIP | 5 | 1.565 | 1162 / 1202 | 96.6% | +1.79 |
| APPSQ | 5 | 0.841 | 1082 / 1202 | 89.9% | +1.25 |

Even when compared only against other pentapeptides, all three finalists remain in the top ~90–99% by iDPPIV composition score (LPPGP top 0.6%, APQIP top 3.4%, APPSQ top 10.1%; all z > +1.2). This shows their priority is **not an artifact of the benchmark's length confounding**: the length-robust residual reranking of §3.9 already preserved their ranks (16→16, 420→420, 107→107), and the within-length-class test independently confirms it. Together they justify using the SCM score for *ranking/soft filtering* while deferring the final priority to multi-signal consensus (§3.7, §4.3).
...
| APPSQ | 5 | 0.380 | 0.394 | 0.236 | 0 | DPP4 single-target #2 (nTar=0) |
...
**Exploratory-scope caveat (mandatory).** We stress that the three axes are *exploratory similarity screens* against small, short-peptide-biased reference sets (N = 37/55/16; Table 8), **not benchmark-validated activity predictors**; their role is hypothesis generation, and any multi-target claim is qualified accordingly (Fan et al., 2024; see also §4.3 limitation 5). LPPGP's multi-target positioning is therefore reported as a lead-generation signal, not as confirmed multi-activity.
...
**Candidate-pool rescoring and consistency** (4,742 SCM-positive candidates): the two models' continuous scores have **Spearman ρ = 0.290** (p≈9×10⁻⁹³) — weak positive correlation; Top-K overlap is only Top-50 4%, Top-100 7%, Top-200 10% — overall clear ranking divergence. This is consistent with both models' known limitations and the benchmark's length confounding, **exactly showing a single in-silico score is insufficient to conclude**, further supporting this paper's multi-signal-consensus strategy.
...
1. **First quantitative exposure of length bias on the iDPPIV-SCM homologous benchmark (Pillar ①):** proved a considerable part of iDPPIV-SCM's accuracy stems from length confounding (pure length baseline ACC 0.820 > SCM 0.771), and proposed a length-robust residual-reranking protocol; finalist candidates proven robust to length bias.
...
3. **MM-GBSA is a pocket-restricted single-structure approximation:** only residues within 9 Å of the active pocket (~11 residues) as the computation domain; long-range receptor–receptor interactions and receptor strain outside the pocket are ignored; −TΔS ignored, receptor glycosylation solvation not counted; for relative ranking among candidates, not absolute-affinity claims. In this study it is deployed only as the **lowest-weight, qualitative vote** within the multi-signal consensus (§3.5, §3.7), never as a standalone decisive criterion.
...
**Data and code availability (persistent archive).** All scripts, benchmark datasets, and intermediate/final products are archived in structured form in the GitHub repository (https://github.com/wu-yijing/moso-bamboo-DPP4-peptides). A persistent archival snapshot (GitHub release + a Zenodo/figshare DOI to be minted at acceptance) accompanies the live repository so that the exact reviewed version remains citable and immutable; the GitHub repository is the primary working copy.

**Ethics statement.** This is a purely computational study that uses only publicly available protein sequences (UniProtKB) and published reference sets; it involves no human or animal subjects, no clinical data, and no proprietary datasets. Therefore, ethics approval is not applicable (N/A).

**Competing interests.** The authors declare that they have no competing financial or non-financial interests.

**CRediT author contributions.** [To be completed by the author(s) before submission:] Conceptualization; Methodology; Software; Validation; Formal analysis; Investigation; Data curation; Writing — original draft; Writing — review & editing; Visualization; Supervision; Project administration; Funding acquisition.

**Generative-AI assistance statement.** During manuscript preparation, large-language-model-based tools were used to assist with language polishing and translation. All scientific content, computational analyses, results, and conclusions were authored, performed, and verified by the human author(s); the AI tools did not generate data or interpret results.
...
3. Zhao H, Gao Z, Wang L, et al. Chromosome-level reference genome and alternative splicing atlas of moso bamboo (*Phyllostachys edulis*). *GigaScience.* 2018;7(11):giy115. doi:10.1093/gigascience/giy115 (PMID 30202850)
4. Peng Z, Zhang C, Zhang Y, et al. Transcriptome sequencing and analysis of the fast growing shoots of moso bamboo (*Phyllostachys edulis*). *PLoS ONE.* 2013;8(11):e78944. doi:10.1371/journal.pone.0078944 (PMC3820679)
...
6. Hiramatsu K, et al. Crystal structures of DPP-IV/inhibitor complexes. *Biol Chem.* 2004;385(7):561–564. doi:10.1515/BC.2004.068
7. UniProt Consortium. UniProt: the Universal Protein Knowledgebase in 2024. *Nucleic Acids Res.* 2024;52(D1):D524–D532. doi:10.1093/nar/gkad1018
8. Trott O, Olson AJ. AutoDock Vina: improving the speed and accuracy of docking with a new scoring function, efficient optimization, and multithreading. *J Comput Chem.* 2010;31(2):455–461. doi:10.1002/jcc.21334
9. Eastman P, et al. OpenMM 7: rapid development of high performance algorithms for molecular dynamics. *PLoS Comput Biol.* 2017;13(7):e1005659. doi:10.1371/journal.pcbi.1005659
10. RDKit: Open-source cheminformatics. https://www.rdkit.org
11. Mooney C, et al. PeptideRanker: a machine-learning tool for the selection of potentially bioactive peptides. *PLoS ONE.* 2012;7(10):e45012. doi:10.1371/journal.pone.0045012 (PMID 23056189)
12. Charoenkwan P, Nantasenamat C, Hasan MM, Moni MA, Lio P, Manavalan B, Shoombuatong W. StackDPPIV: a novel computational approach for accurate prediction of dipeptidyl peptidase IV (DPP-IV) inhibitory peptides. *Methods.* 2022;204:189–198. doi:10.1016/j.ymeth.2021.12.001 (PMID 34883239)
13. Wang D, Jin J, Li Z, Wang Y, Fan M, Liang S, Su R, Wei L. StructuralDPPIV: a novel deep learning model based on atom structure for predicting dipeptidyl peptidase-IV inhibitory peptides. *Bioinformatics.* 2024;40(2):btae057. doi:10.1093/bioinformatics/btae057
14. Zhang J, Wang R, Zhou K, Xiao T, Zhu L, Min Y, Wang Y. PepBenchmark: a standardized benchmark for peptide machine learning. *arXiv.* 2026;arXiv:2604.10531. doi:10.48550/arXiv.2604.10531
15. Lacroix IME, Li-Chan ECY. Evaluation of the potential of dietary proteins as precursors of dipeptidyl peptidase (DPP)-IV inhibitors by an in silico approach. *J Funct Foods.* 2012;4(2):403–422. doi:10.1016/j.jff.2012.01.008
16. Kalyan G, et al. An antihypertensive peptide predictor for ranking angiotensin-I-converting-enzyme inhibitory peptides. *J Agric Food Chem.* 2021;69(49):14995–15004. doi:10.1021/acs.jafc.1c04555 (PMID 34855377)
17. Lu H, et al. α-Glucosidase inhibitory peptides from food proteins: a review of preparation, identification, and structure–activity relationships. *Nutrients.* 2023;15(19):4267. doi:10.3390/nu15194267 (PMID 37836551)
18. Wang E, et al. MM/PBSA and MM/GBSA: what are the important proteins? A practical case study on the periplasmic chaperone PapD. *Chem Rev.* 2019;119(16):9478–9508. doi:10.1021/acs.chemrev.9b00055 (PMID 31244000)
19. Zhang M, et al. Food-derived dipeptidyl peptidase-IV inhibitory peptides: a review of their production, identification, and bioavailability. *Crit Rev Food Sci Nutr.* 2024;64(8):2053–2075. doi:10.1080/10408398.2022.2120454
20. Du Z, et al. Bioinformatics approaches to discovering food-derived bioactive peptides: a review. *TrAC Trends Anal Chem.* 2023;162:117051. doi:10.1016/j.trac.2023.117051
21. Liu Y, Zhang Y, Liu M, Yu S, Tian Z, Li W, Han W. Virtual hydrolysis-based screening of wheat-derived DPP-IV inhibitory peptides: a mechanistic analysis integrating cell experiments and molecular dynamics simulations. *J Agric Food Chem.* 2025;73(31):19410–19426. doi:10.1021/acs.jafc.5c03006 (PMID 40623964)
22. Blanes-Mira C, Fernández-Aguado P, de Andrés-López J, Fernández-Carvajal A, Ferrer-Montiel A, Fernández-Ballester G. Comprehensive survey of consensus docking for high-throughput virtual screening. *Molecules.* 2023;28(1):175. doi:10.3390/molecules28010175 (PMID 36615367)
23. Ajayi FF, Mudgil P, Jobe A, Antony P, Vijayan R, Gan CY, Maqsood S. Novel plant-protein (quinoa) derived bioactive peptides with potential anti-hypercholesterolemic activities: identification, characterization and molecular docking of bioactive peptides. *Foods.* 2023;12(6):1327. doi:10.3390/foods12061327 (PMID 36981252)
24. He B, Lian Y, Xue H, Zhou Y, Wei Y, Ma J, Tan Y, Wu Y. DPP-IV inhibitory peptide against in vitro gastrointestinal digestion derived from goat's milk protein and its activity enhancement via amino acid substitution. *Foods.* 2024;13(17):2721. doi:10.3390/foods13172721 (PMID 39272487)
25. Gao X, Zhang C, Wang N, Lin JM, Dang Y, Zhao Y. Screening of oral potential angiotensin-converting enzyme inhibitory peptides from *Zizyphus jujuba* proteins based on gastrointestinal digestion in vivo. *Int J Mol Sci.* 2023;24(21):15848. doi:10.3390/ijms242115848 (PMID 37958831)
26. Rahmi A, Arcot J. In vitro assessment methods for antidiabetic peptides from legumes: a review. *Foods.* 2023;12(3):631. doi:10.3390/foods12030631
27. Chen D, Shen X, Chen W, Wu D, Zhang Z, Liu P, Liu Y, Li W, Yang Y. A screening strategy for bioactive peptides from enzymolysis extracts of *Lentinula edodes* based on molecular docking and molecular dynamics simulation. *J Future Foods.* 2025;5(4):388–397. doi:10.1016/j.jfutfo.2024.07.017
28. Fan S, Liu Q, Du Q, Zeng X, Wu Z, Pan D, Tu M. Multiple roles of food-derived bioactive peptides in the management of T2DM and commercial solutions: a review. *Int J Biol Macromol.* 2024. doi:10.1016/j.ijbiomac.2024.134993
...
- `scripts/phaseA/length_agnostic_ranking.py` + `data/phaseA/length_agnostic_ranking.tsv` / `_summary.txt` — **§3.9.2 length-agnostic (within-length-class) ranking of the three finalists (robustness check)**
