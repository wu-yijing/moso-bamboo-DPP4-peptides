# -*- coding: utf-8 -*-
"""配方探测 v3: 长度混杂分析 + 长度归一化 SCM + odds-ratio 版"""
...
def acc_at(seqs,labs,preds): return sum(1 for y,p in zip(labs,preds) if p==y)/len(labs)
...
# ---- 构建两种全局卡: Obs/Exp 与 Odds(pos/neg) ----
...
def sc_sum(seq,P): return sum(P.get(a,0.0) for a in seq)
def sc_mean(seq,P): return sc_sum(seq,P)/len(seq) if seq else 0.0
...
for mode in ["obs/exp","odds"]:
    for agg in ["sum","mean"]:
        ...
        t=tune(trS,trL,s_tr)
        print(f"[全局 {mode} {agg}] 阈值={t:.3f} trainACC={acc_at(trS,trL,[1 if x>t else 0 for x in s_tr]):.3f} testACC={acc_at(teS,teL,[1 if x>t else 0 for x in s_te]):.3f}")
