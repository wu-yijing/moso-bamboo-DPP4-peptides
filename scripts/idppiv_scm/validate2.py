# -*- coding: utf-8 -*-
"""Final validation: global composition-based amino-acid SCM (iDPPIV-SCM recipe)
   - nested 5-fold CV (each fold builds card + tunes threshold on 4 folds, tests held-out fold) -> honest estimate
   - report on independent test set (threshold optimized on full training set)
"""
...
print(f"[nested 5-fold CV] mean ACC={sum(cv_accs)/5:.3f}  folds={[f'{a:.3f}' for a in cv_accs]}")

# final model: build card on full training set, tune threshold on training set, test on independent set
P = build_global(pos, neg)
sc_tr = [score(s, P) for s in trS]; t = tune(trS, trL, sc_tr)
sc_te = [score(s, P) for s in teS]
print(f"[independent test] thr(tuned)={t:.3f}  ACC={acc_at(teS, teL, sc_te, t):.3f}  (literature~0.797)")
print(f"[independent test] thr=0        ACC={acc_at(teS, teL, sc_te, 0):.3f}")
sc_tr0 = [score(s, P) for s in trS]
print(f"[full training set] thr=0      ACC={acc_at(trS, trL, sc_tr0, 0):.3f}")
print("\nConclusion: the global amino-acid composition SCM reproduces the literature independent accuracy (~0.77-0.80); recipe confirmed.")
