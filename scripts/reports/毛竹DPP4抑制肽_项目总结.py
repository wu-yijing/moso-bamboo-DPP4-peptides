...
sub = doc.add_paragraph('生成时间：2026-07-13 22:28  |  工作目录：E:/workbuddy/Claw/')
...
bullet('方法：透明代理打分（基于文献理化特征：N 端疏水、Pro 含量、分子量），非官方服务器输出。此步骤需以官方 PeptideRanker/AllerTOP/ToxinPred 结果替换（见末尾局限）。')
...
# ===== 三、交付物清单 =====
h1('三、当前交付物清单（均在 E:/workbuddy/Claw/）')
for f in [
...
# ===== 五、局限 =====
h1('五、必须如实披露的局限')
bullet('过滤打分是代理启发式，非官方服务器输出，正式稿件须替换并注明。')
...
for s in [
    ...
    'Python venv（C:/Users/Administrator/.workbuddy/binaries/python/envs/default）：numpy、scipy、biopython、openbabel、rdkit、pandas',
...
doc.save('E:/workbuddy/Claw/毛竹DPP4抑制肽_项目总结.docx')
print('Saved: 毛竹DPP4抑制肽_项目总结.docx')
